...
const postProcess = async (tempPath, destinationPath) => {
  const unpackedSourcePath = join(tempPath, 'zip', 'build');
  const packedSourcePath = join(tempPath, 'packed.zip');
  const packedDestPath = join(destinationPath, 'packed.zip');
  await move(unpackedSourcePath, destinationPath); // Copy built files to destination
...
const main = async (manifestPath, destinationPath) => {
  try {
    ...
    console.log(chalk.green('\nThe Electron extension has been built!'));
    console.log(chalk.green('You can test this build by running:'));
    console.log(chalk.gray('\n# Using the require(\'../electron-react-devtools\').install();'));
...
main(join(__dirname, '..', 'manifest.json'), join(__dirname, '..', 'build'));
